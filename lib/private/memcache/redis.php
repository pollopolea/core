...
class Redis extends Cache implements IMemcacheTTL {
	...
	public function cas($key, $old, $new) {
		...
		$this->instance->transaction([
			'cas' => true,
			'watch' => $this->getNameSpace() . $key
		], function(MultiExec $tx) use ($key, $old, $new, &$result) {
			$tx->multi();
			$existing = json_decode($tx->get($this->getNameSpace() . $key), true);
			if ($existing === $old) {
				$tx->set($this->getNameSpace() . $key, $new);
				$result = true;
			}
		});
		return $result;
	...
	public function cad($key, $old) {
		...
		$this->instance->transaction([
			'cas' => true,
			'watch' => $this->getNameSpace() . $key
		], function (MultiExec $tx) use ($key, $old, &$result) {
			$tx->multi();
			$existing = json_decode($tx->get($this->getNameSpace() . $key), true);
			if ($existing === $old) {
				$tx->del([$this->getNameSpace() . $key]);
				$result = true;
			}
		});
		return $result;
